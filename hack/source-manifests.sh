...
function gen_ocs_csv() {
	...
	mv "$(git rev-parse --show-toplevel)/bundle/manifests/ocs-operator.clusterserviceversion.yaml" "$OCS_CSV"
...
mv "$(git rev-parse --show-toplevel)/bundle/manifests" "$OCS_FINAL_DIR"
mv "$(git rev-parse --show-toplevel)/bundle/metadata" "$(dirname $OCS_FINAL_DIR)"/metadata
rm -rf "$(git rev-parse --show-toplevel)/bundle"
rm "$(git rev-parse --show-toplevel)/bundle.Dockerfile"
